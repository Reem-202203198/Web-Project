  ...
  .then((user) => {
    ...
    if (user.profilePicture) {
      localStorage.setItem(
        "currentUser",
        JSON.stringify({ ...currentUser, profilePicture: user.profilePicture }),
      );
    }
  });
...
function createPostCard(post, author) {
  ...
  const isOwner =
    ...
    String(post.author?.id || post.authorId || post.userId);
    ...
    `;
      ...
      if (deleteBtn) {
        deleteBtn.addEventListener("click", async function () {
          commentEl.remove();
          const currentCount =
            parseInt(commentLink.textContent.replace("💬", "")) || 0;
...
async function loadSuggestedUsers() {
  const container = document.querySelector(
    ".right-sidebar .side-card:last-child",
  );
  if (!container) return;
  ...
  try {
    ...
    users.forEach((user) => {
      ...
      if (followingIds.includes(user.id)) return;
      ...
      userEl.innerHTML = `
        <div class="suggested-left">
          ...
          <span class="user-link" style="cursor:pointer;">${user.username}</span>
        </div>
        <button class="follow-btn">Follow</button>
      `;

      userEl.querySelector(".user-link").addEventListener("click", () => {
